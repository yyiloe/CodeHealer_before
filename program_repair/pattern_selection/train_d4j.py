...
if __name__ == "__main__":
    ...
    word2vec = Word2Vec.load(os.path.join(root, "train/embedding/w2v_512")).wv
    embeddings = np.zeros((word2vec.syn0.shape[0] + 2, word2vec.syn0.shape[1]), dtype="float32")

    TOKEN_LENGTH = 400
    HIDDEN_DIM = 80
    EPOCHS = 15   #8 #9 #15 15
    BATCH_SIZE = 32  #32 #8 #32 32
    LABELS = 11
    USE_GPU = True
    ...
    #drop 0.2 0 0.1 0.1 head 1 4 8 32 l0.0001 0.0001 0.0001 0.0001
    
    model = MultiClassifier(embeddings, EMBEDDING_DIM, MAX_TOKENS,400,0.1,1, 11, 32)
    print("hello")
    model.load_state_dict(torch.load("./model_save/model_params.pkl"))
    #model.load_state_dict(torch.load("./model_save_d4j/5/model_params.pkl"))
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch + 1 == EPOCHS:
            model_save_dir = "/home/dancher/Programs/Projects/trans2/TRANSFER-master/program_repair/automatic_fix/dnn_model/model_save_d4j/"
    ...
    model.load_state_dict(torch.load("/home/dancher/Programs/Projects/trans2/TRANSFER-master/program_repair/automatic_fix/dnn_model/model_save_d4j/{}/model_params.pkl".format(part)))
    model.eval()
    fix_count = 0
    cannot_fix_list = []
 ...
 #       a=[factor for factor in cannot_fix_list]
 #       b=[test_bugs[factor] for factor in cannot_fix_list]
 ###       for i in (0,len(a)):
 #        print(output.tolist()[i])
 #        print(b[i])
